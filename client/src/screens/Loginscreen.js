...
export default function Loginscreen() {
  ...
  // Getting login state from the redux store using useSelector hook
  const loginstate = useSelector((state) => state.loginUserReducer);
  // Destructuring loading and error from loginstate object
  const { loading, error } = loginstate;
  // Getting dispatch function from useDispatch hook
  const dispatch = useDispatch();

  // useEffect hook to check if the user is already logged in, if yes redirect to home page
  useEffect(() => {
...
// Function to handle login button click
  ...
  // Return statement with login form
  return (
    <div className="login ">
