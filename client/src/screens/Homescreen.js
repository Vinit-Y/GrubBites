...
export default function Homescreen() {
  ...
  // Select the pizzas state from the redux store
  const pizzasstate = useSelector((state) => state.getAllPizzasReducer);

  // Extract pizzas, error, and loading values from the pizzas state
  const { pizzas, error, loading } = pizzasstate;

  // Dispatch the getAllPizzas action when the component mounts
  useEffect(() => {
    dispatch(getAllPizzas());
  }, []);

  return (
    // Render a container with a background image and color
