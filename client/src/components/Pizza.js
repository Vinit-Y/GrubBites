...
export default function Pizza({ pizza }) {
  // Initialize AOS library for animation
  AOS.init({});

  // Initialize state for quantity, varient and modal show/hide
  const [quantity, setquantity] = useState(1);
  const [varient, setvarient] = useState("small");
  const [show, setShow] = useState(false);

  // Define functions to handle modal show and hide
  ...
  function addtocart() {
    // Define function to add the selected pizza to cart
